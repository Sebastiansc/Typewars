...
export default class Bullet{
  ...
  target(ship, distance, shiftBullet){
    const x = this.setXCoordinate(ship);
    createjs.Tween.get(this.bullet).
      to({x: x, y: ship.container.y, alpha: 1}, distance / this.speed).
        call(() => this.hitTarget(ship, shiftBullet));
  }

  setXCoordinate(ship){
    if(ship.container.x > cannon.position()[0] + 100){
  ...
  hitTarget(ship, shiftBullet){
    ...
    ship.explosion();
    shiftBullet();
    if(!ship.word && cannon.bullets.length <= 1){
